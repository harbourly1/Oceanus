    ...
    },
    {
      ...
      render: (r: any) => (
        <div className="flex gap-2">
          <Link href={`/admin/allocation/${r.id}`}>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <Link href="/admin/allocation/unassigned">
            <Button size="sm" variant="secondary">Unassigned Queue</Button>
          </Link>
          <Link href="/admin/allocation/logs">
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Create Allocation Pool">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Input label="Max Daily Leads" type="number" min={1} max={100} value={form.maxDailyLeads}
              onChange={(e) => setForm({ ...form, maxDailyLeads: Number(e.target.value) })} />
            <Input label="Max Weekly Leads" type="number" min={1} max={500} value={form.maxWeeklyLeads}
              onChange={(e) => setForm({ ...form, maxWeeklyLeads: Number(e.target.value) })} />
          </div>
          <div>
            <label className="block text-sm font-medium mb-2" style={{ color: 'var(--color-text-secondary)' }}>Agents *</label>
            <div className="flex flex-wrap gap-2">
              ...
              {agents.length === 0 && <p className="text-sm text-gray-500 mt-2">No sales agents found</p>}
            </div>
          </div>
          <div className="flex justify-end gap-2 pt-2">
            <Button variant="secondary" type="button" onClick={() => setShowCreate(false)}>Cancel</Button>
            <Button type="submit" loading={createPool.isPending}
