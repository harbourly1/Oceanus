...
export default function SalesInvoicesPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold" style={{ color: 'var(--color-text-primary)' }}>My Invoices</h1>
        <p className="text-sm mt-1" style={{ color: 'var(--color-text-muted)' }}>Track invoices you have created and their approval status</p>
      </div>
      ...
      {invoices.length === 0 ? (
        <EmptyState title="No invoices" message="You haven't created any invoices yet" />
      ) : (
        <Card padding={false}>
            ...
            <table className="w-full text-left">
              <thead>
                <tr style={{ borderBottom: '1px solid var(--color-border-default)' }}>
                  {['Invoice #', 'Customer Name', 'Insurance Provider', 'Type', 'Total Premium', 'Payment Date', 'Payment Mode', 'Sales Agent', 'Status', 'Created'].map(h => (
