      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>
            {pool._count?.assignmentLogs || 0} leads assigned through this pool
          </p>
        </div>
        ...
        <div className="rounded-xl p-6" style={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border-default)' }}>
          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div className="flex justify-between pt-4">
              ...
              <Button
                type="submit"
                loading={updatePool.isPending}
                disabled={form.productTypes.length === 0 || form.agentIds.length === 0}
              >
                Save Changes
              </Button>
            </div>
        ...
        <div className="rounded-xl p-6" style={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border-default)' }}>
          ...
          <div className="space-y-3">
            {pool.agents?.length === 0 && (
              <p className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>
                No agents assigned to this pool
              </p>
            )}
              ...
              >
                <div>
                  ...
                  <p className="text-xs" style={{ color: 'var(--color-text-secondary)' }}>
                    Last assigned: {pa.lastAssignedAt ? new Date(pa.lastAssignedAt).toLocaleString() : 'Never'}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <span
                    ...
                    style={{
                      background: pa.user?.isOnLeave ? 'rgba(239, 68, 68, 0.15)' : 'rgba(16, 185, 129, 0.15)',
                      color: pa.user?.isOnLeave ? '#ef4444' : '#10b981',
                    }}
                  >
                  ...
                  <Button
                    size="sm"
                    variant="secondary"
                    onClick={() => toggleLeave(pa.userId, pa.user?.isOnLeave || false)}
