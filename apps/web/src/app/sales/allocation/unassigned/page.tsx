  ...
  const columns = [
    {
      ...
      header: 'Lead',
      render: (r: any) => (
        <Link href={`/sales/leads/${r.id}`} className="text-blue-500 hover:underline font-medium">
          {r.ref}
        </Link>
      ),
    },
      ...
      render: (r: any) => {
        ...
        return (
          <span className="text-sm flex items-center gap-1">
            <ProductIcon iconKey={config?.iconKey} size={14} /> {config?.label || r.productType}
          </span>
        );
      },
    },
    { key: 'fullName', header: 'Contact' },
    {
      ...
      render: (r: any) => {
        ...
        const colors: Record<string, string> = {
          hot: 'var(--color-accent-red)',
          warm: 'var(--color-accent-amber)',
          cold: 'var(--color-accent-blue)',
        };
        return (
          <span className="text-sm font-medium" style={{ color: colors[temp] }}>
            {r.score} ({temp.toUpperCase()})
          </span>
        );
      },
    },
      ...
      render: (r: any) => (
        <div className="flex gap-2 items-center">
          <Button
            ...
            loading={retryAllocation.isPending}
          >
            Auto-Allocate
          </Button>
          {assigning === r.id ? (
            <Select
              value=""
              onChange={(e) => {
                if (e.target.value) handleManualAssign(r.id, e.target.value);
              }}
              options={[
            ...
            <Button size="sm" variant="secondary" onClick={() => setAssigning(r.id)}>
              Assign
            </Button>
          )}
        </div>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>
            {meta.total} leads awaiting assignment
          </p>
        </div>
        <Link href="/sales/allocation">
          <Button size="sm" variant="secondary">Back to Pools</Button>
        </Link>
      </div>
      ...
      {meta.totalPages > 1 && (
        <div className="flex justify-center gap-2">
          ...
          <span className="py-2 px-4 text-sm" style={{ color: 'var(--color-text-secondary)' }}>
